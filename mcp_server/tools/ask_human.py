...
Note: this tool relies on LangGraph's ``interrupt`` and only works inside a
running graph. It is kept here for parity with the application's toolset; it
will return an error when called from a bare MCP session.
"""
