...
def _route_after_intent(state: GraphState) -> str:
    """Hand off to the agent once the inbound intent check passes.

    The agent owns correctness/security/performance review from here: they
    are tools it can call freely (individually, all three, or none), not a
    forced fan-out stage the graph runs before the agent gets a turn.
    """
    return "agent" if state.get("is_safe_intent", False) else "guardrail_redirect"
...
def _merge_new_findings(existing: list[Any], new_findings: list[Any]) -> list[Any]:
    """Merge newly-returned review-tool findings into the running, ordered list.

    Called from the agent's own tool-execution step whenever the model chose
    to call one or more review tools this turn, replacing the old dedicated
    ``merge_reviews`` graph node.
    """
    merged = list(existing) + list(new_findings)
    return sorted(
        merged,
        key=lambda finding: (
...
class LangGraphAgent:
    ...
    def __init__(self):
        ...
        # only supplies the current unbound model instance. This includes
        # the correctness/security/performance review tools, so the ReAct
        # loop -- not a fixed graph stage -- decides which of them to call.
        self.agent_tools = list(bound_tools)
    ...
    async def _agent(self, state: GraphState | dict[str, Any], config: RunnableConfig) -> Command:
        ...
        if isinstance(last_message, AIMessage) and last_message.tool_calls:
            ...
            async def _execute_tool(tool_call: dict) -> ToolMessage:
                ...
                # correctness/security/performance reviews need the current
                # graph state (code, language, sanitized_code, ...) rather
                # than model-supplied args. Inject it directly instead of
                # trusting the model to reconstruct it -- the model only
                # ever sees the tool name and docstring for these.
                if tool_call["name"] in STATE_INJECTED_REVIEW_TOOLS:
                    tool_args = {"state": state_values}

                tool_result = await tool.ainvoke(tool_args)

                if tool_call["name"] in STATE_INJECTED_REVIEW_TOOLS:
                    try:
                        new_findings.extend(json.loads(tool_result) or [])
                    except (TypeError, ValueError):
                        logger.warning("review_tool_findings_unparseable", tool=tool_call["name"])
            ...
            # When the model calls two or three review tools in the same
            # turn, they run concurrently here -- that's the "parallel"
            # execution now; it's driven by the model's choice, not a fixed
            # graph fan-out.
            ...
            update: dict[str, Any] = {"messages": outputs}
            if new_findings:
                update["review_findings"] = _merge_new_findings(
                    state_values.get("review_findings") or [], new_findings
                )
    ...
    async def create_graph(self) -> Optional[CompiledStateGraph]:
        ...
        and an outbound safety perimeter. Correctness/security/performance
        review are tools bound to the agent, not graph nodes -- the agent
        decides which (if any) to call each turn, and calling more than one
        in the same turn runs them concurrently:

            inbound_dlp -> inbound_intent -> agent <-> agent -> outbound -> END
                  |              |             |
                  +--------------+--> redirect -+
        ...
        if self._graph is None:
            try:
                # web_search / ask_human / memory_search are served by the
                # MCP server (mcp_server/server.py) now, not native tools.
                # Fetch them once here -- graph creation is already async and
                # only runs once per agent instance -- rather than per
                # request. If the MCP server can't be reached, get_mcp_tools
                # fails closed to an empty list and the agent still gets
                # everything else it has locally.
                mcp_tools = await get_mcp_tools()
                self.agent_tools = list(self.agent_tools) + mcp_tools
                self.tools_by_name = {tool.name: tool for tool in self.agent_tools}
