...
class PerformanceReviewResult(BaseModel):
    """Wrapper so the LLM call has a single schema to target.
 
    `with_structured_output` needs one top-level model, not a bare list.
    """
...
class PerformanceFindingDraft(BaseModel):
    """Internal, stricter shape the LLM actually targets.
 
    Adds a real, Pydantic-validated subtype field on top of the shared
    Finding fields — this is what makes PerformanceIssueType/StyleSubtype
    ACTIVE rather than decorative: if the LLM returns a subtype not in
    either enum, this model fails to validate instead of silently
    accepting free text.
    """
...
class PerformanceReviewDraft(BaseModel):
    """What we actually request via `response_format=`.
 
    Converted to PerformanceReviewResult (shared Finding shape) after
    validation.
    """
...
def _draft_to_finding(draft: PerformanceFindingDraft) -> Finding:
    """Fold the validated subtype into `message` and drop to the shared shape.
 
    The subtype only ever reaches this function after Pydantic has already
    confirmed it's a real enum value.
    """
